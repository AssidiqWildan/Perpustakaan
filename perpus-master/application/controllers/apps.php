...
class Apps extends CI_Controller {
	...
	public function buku() {
		...
		if ($mau_ke == "del") {
			...
			$this->db->query("INSERT INTO t_buku VALUES ('', '$id_kelas', '$id_jenis', '$judul', '$pengarang', '$penerbit', '$th_terbit', '$isbn', '$jml_hal', '$asal_perolehan', '$harga', '$id_lokasi', '$stat', 'RR', 'R', '$deskripsi', NOW())");
	...
	public function Kelola() {
		...
		$jumlah = $this->db->query("SELECT * FROM admin WHERE u = '$user' AND level=1")->num_rows();
		...
		$config['base_url'] 	= base_URL().'apps/Kelola/p/';
		$config['total_rows'] 	= $total_rows;
		$config['uri_segment'] 	= 4;
		$config['per_page'] 	= 10; 
		$config['num_tag_open'] = '<li>';
		...
		$cari					= addslashes($this->input->post('q'));
		//view tampilan website\
		$a['data']				= $this->db->query("SELECT * FROM admin LIMIT $awal, $akhir ")->result();
		$a['page']				= "kelola_user";

		if ($mau_ke == "del") {
			$this->db->query("DELETE FROM admin WHERE id = '$idu'");
			
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\">Data has been deleted </div>");
			redirect('apps/Kelola');
		} else if ($mau_ke == "cari") {
			$a['data']		= $this->db->query("SELECT * FROM admin WHERE judul LIKE '%$cari%' OR penerbit LIKE '%$cari%' OR pengarang LIKE '%$cari%' OR deskripsi LIKE '%$cari%' ORDER BY id DESC")->result();
			$a['page']	= "kelola_user";
		} else if ($mau_ke == "add") {
			$a['page']	= "f_Kelola";
		} else if ($mau_ke == "edt") {
			$idu = $this->uri->segment(4);
			$a['datpil']		= $this->db->query("SELECT * FROM admin WHERE id = '$idu'")->row();	
			$a['page']			= "f_Kelola";
		}else if ($mau_ke == "act_add") {
			$this->db->query("INSERT INTO admin VALUES ('', '$nama', '$u', '$p', '$level')");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\">Data has been added</div>");
			redirect('apps/Kelola');
		} else if ($mau_ke == "act_edt") {
			$this->db->query("UPDATE admin SET  name = '$nama', u = '$u', p='$p', level = '$level' WHERE id = '$id'");
	...
	public function anggota() {
		...
		if ($mau_ke == "del") {
			...
			$this->db->query("DELETE FROM t_anggota WHERE nim_nidn = '$nim_nidn'");
			...
			$nim_nidn = $this->uri->segment(4);
			$a['datpil']		= $this->db->query("SELECT * FROM t_anggota WHERE nim_nidn = '$nim_nidn'")->row();	
			$a['page']			= "f_anggota";
		}else if ($mau_ke == "act_add") {
			$this->db->query("INSERT INTO t_anggota VALUES ('$nim_nidn', '$nama', '$alamat', '$jk', '$agama', '$tmp_lahir', '$tgl_lahir', '',  NOW(), '$jenis', '1')");
			
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\">Data has been added</div>");
			redirect('apps/anggota');
		} else if ($mau_ke == "act_edt") {
			$this->db->query("UPDATE t_anggota SET nama = '$nama', alamat = '$alamat', jk = '$jk', agama = '$agama', tmp_lahir = '$tmp_lahir', tgl_lahir = '$tgl_lahir', jenis = '$jenis', stat = '$status' WHERE nim_nidn = '$nim_nidn'");

			$this->session->set_flashdata("k", "<div class=\"alert alert-success\">Data has been updated</div>");			
			redirect('apps/anggota');
		} else if ($mau_ke == "list_pinjam") {
			$a['id_anggota']= $nim_nidn;
			$a['data']		= $this->db->query("SELECT t_trans.*, t_buku.judul 
												FROM t_trans 
												LEFT JOIN t_buku ON t_trans.id_buku = t_buku.id
												WHERE t_trans.id_anggota = '$nim_nidn' ORDER BY t_trans.id DESC")->result();
			$a['page']		= "d_lis_pinjam";
		} else {
			$a['page']	= "d_anggota";
		}
	...
	public function trans() {
		...
		if ($mau_ke == "pilih_anggota") {
			...
			if ($cek_peminjam > 0) {
				...
				$a['det_anggota']	= $this->db->query("SELECT * FROM t_anggota WHERE nim_nidn = '$id_anggota'")->row();
			...
			for ($i = 1; $i <= $jml_buku; $i++) {
				$id_buku = $this->input->post('id_buku_'.$i);

				$tersedia = $this->db->query("SELECT id FROM t_trans WHERE tgl_pinjam <= '$sekarang' AND tgl_kembali >= '$sekarang' AND id_buku = '$id_buku' AND stat = 'P'")->num_rows();

				if ($tersedia !== 0) {
					$this->session->set_flashdata("k", "<div class=\"alert alert-danger\">buku sudah dipinjam </div>");
					redirect('apps/trans');
				}
			}
	...
	public function stat() {
		...
		$a['anggota_paling_banyak_meminjam']	= $this->db->query("SELECT t_trans.id_buku, t_anggota.nama, COUNT( t_trans.id_anggota ) AS jumlah_pinjam FROM t_trans LEFT JOIN t_anggota ON t_trans.id_anggota = t_anggota.nim_nidn GROUP BY id_buku ORDER BY jumlah_pinjam DESC")->result();
	...
	public function l_peminjam() {
		...
		$m['p_hariini']	= $this->db->query("SELECT a.*, c.nama nmanggota, b.judul FROM t_trans a
											...
											LEFT JOIN t_anggota c ON a.id_anggota = c.nim_nidn 
